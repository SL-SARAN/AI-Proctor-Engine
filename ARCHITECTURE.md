...
**Production deployment** (turn N+11). All client and server layers
are complete: the data model, ingestion, preprocessing, inference
(server-side and client-side), fusion, evidence, orchestration,
browser client with capture, client-side inference, and now the
Redis-backed launch-state store. The remaining unchecked item is
the live Kubernetes cluster provisioning and end-to-end smoke test
on a real cluster. The manifests under `k8s/00-…` through `k8s/10-…`
...
| 2026-08-01 | **Browser client skeleton + token delivery fix (turn N+8).** New `client/` package: Vanilla TypeScript + Vite + Vitest. 63 client-side tests pass. `ws-client.ts` implements the authenticated WS handshake with automatic reconnect and ping/pong; `browser-events.ts` binds the six valid DOM events; `rolling-buffer.ts` implements a 10s in-memory circular buffer keyed by capture timestamp; `kill-switch.ts` locks UI, drains the buffer, and renders the "pending review" (not "terminated") message per the `docs/05` fast-track undo logic. `envelope.ts` shares the typed structure with the backend. **Session-token delivery fixed at both ends:** the token no longer travels in any query string. The LTI route (`src/proctoring_engine/lti/service.py`) now issues its redirect carrying both `session_token` and `session_id` in the URL fragment (`#`); the client reads them and immediately clears the fragment via `history.replaceState`. The WebSocket connection (`src/proctoring_engine/websocket/routes.py`) now demands the token exclusively via the `Sec-WebSocket-Protocol` subprotocol header (`proctoring-v1.<jwt>`); a query-param token is rejected with `WS_CLOSE_AUTH_FAILED` (no fallback, to permanently close the proxy access-log leak). 92 WS tests + 31 LTI tests updated to assert this. `api.py` extended to mount the built client bundle at `/client/` via `StaticFiles`. Dockerfile converted to multi-stage to run `npm run build` alongside the Python environment. `.github/workflows/ci.yml` expanded with a parallel `client-ci` job. Total test count: 639 Python + 63 JS = 702. Next layer: client-side inference (turn N+9). | Antigravity |
| 2026-08-02 | **Client-side inference landed (turn N+9).** Three new modules added to the client layer: `media-capture.ts` (`getUserMedia` wrapper with constraint handling, frame extraction, and JPEG base64 encoding without data URL prefixes); `face-inference.ts` (`FaceInferenceRunner` integrating MediaPipe Tasks Vision `@mediapipe/tasks-vision`, lazy async initialization of `FaceDetector` and `FaceLandmarker` using `FilesetResolver.forVisionTasks()`, extracting highest-confidence detection and 478 face landmarks containing blendshapes); `capture-loop.ts` (`CaptureLoop` class orchestrating `requestAnimationFrame` using a dual interval configuration: light frames processing face presence locally every 1s, heavy frames capturing a JPEG and identifying gaze via landmarks every 2.5s). Gaze tracking uses a heuristic based on tracking left/right iris centers (`LEFT_IRIS_CENTER = 468`, `RIGHT_IRIS_CENTER = 473`) against eye corners. The `RollingBuffer` was adapted with a new `HeavyFrameEntry` interface and an `add` method providing legacy entry generation with new properties such as the `jpegBase64` and normalized bounding boxes mapped by video resolution. Fully integrated into `main.ts` utilizing `ws-client.ts` hooks on successful connection status. The build fixes index pathing inside `index.html`. 113 total Vitest client tests passing (covering async MediaStream hooks, test doubles representing DOM and media prototypes). Next layer: Live cluster provisioning + end-to-end smoke test on a real Kubernetes cluster (turn N+10). | Antigravity |
| 2026-08-02 | **Redis-backed `LaunchStateStore` landed (turn N+10) + a stale CONTEXT.md doc fix.** Resolves the live multi-replica bug flagged in `docs/DEPLOYMENT.md` §6.2: at the documented starting sizing of 2 API replicas, the process-local launch-state store is already incorrect — a state/nonce issued by replica A is invisible to replica B if the load balancer doesn't pin the OIDC callback to the same replica. **Doc fix (Part 1):** CONTEXT.md §4 listed "WebSocket affinity / gateway architecture" as an open decision. The other three files (`SYSTEM_STATE.md` §10, `docs/00-index-and-architecture-flow.md` §"What's been decided", `docs/DEPLOYMENT.md` §6.1) had already recorded the resolution as Cloudflare Durable Objects (2026-07-25); the CONTEXT.md entry was stale. Updated to match the three other files. The actual remaining open decisions in CONTEXT.md §4 are now correctly narrowed to `PolicyConfig.name` uniqueness and identity-match runtime-failure handling. **Code layer (Part 2):** refactored `src/proctoring_engine/lti/state.py` to extract `LaunchStateStore` as a `Protocol`, rename the original concrete class to `InMemoryLaunchStateStore`, and add `RedisLaunchStateStore` (async). Atomic consume is enforced by a Lua script (`EVALSHA`-cached at construction) so the same state cannot be consumed twice across concurrent callers in different replicas. Two-store cross-replica visibility is verified by a dedicated test (`test_two_stores_share_state_via_fake_server`) that points two `RedisLaunchStateStore` instances at the same `FakeAsyncRedis` server. Atomicity under contention is verified by `test_concurrent_consume_runs_through_lua_atomicity` (16 concurrent consumes → exactly 1 success). The route handlers in `lti/routes.py` now `await` the store calls uniformly so swapping between the two backends is a configuration decision (the `LTI_STATE_STORE_BACKEND` and `REDIS_URL` env vars), not a code change. The `InMemoryLaunchStateStore` methods were converted to `async def` so the same `await` syntax works against both implementations; existing tests in `test_lti_state.py` and `test_lti_routes.py` were updated to match. New dep `redis>=5.0,<6`; dev-only `fakeredis[lua]>=2.20,<3` for tests. 14 new unit tests (Lua script, atomicity under contention, cross-replica visibility, TTL expiry, peek/consume independence). Total: 653 Python unit + 113 client tests passing. Next layer: live K8s cluster provisioning + e2e smoke test on a real cluster (turn N+11). | Antigravity |
| 2026-08-15 | **Aggregator handles identity-match and audio-vad (turn 8).** SessionAggregator grew `process_identity_match` and `process_audio_vad` paths wired to `TelemetryModality.IDENTITY_MATCH` and `TelemetryModality.AUDIO_VAD`. Identity match uses cosine similarity against the pre-loaded `EnrollmentReference` embedding; audio VAD uses the speech ratio vs `audio_speech_ratio_threshold` plus `audio_noise_floor_dbfs` for elevated-RMS events. Path-2 / Path-3 ladder integration tested with all three rule severities (LOW / MEDIUM / CRITICAL). | Claude |
| 2026-08-16 | **`FrameDispatcher` wires the pipeline (turn 9a).** New `src/proctoring_engine/orchestration/_frame_dispatcher.py`: per-session background `threading.Thread` that drains the WebSocket-layer `TelemetryEventBuffer` via `pop_blocking(timeout=0.1)`, dispatches heavy frames through the `SessionAggregator`, and emits `FlagDecision`s onto a thread-safe `queue.Queue`. `BufferedEvent.synthetic_id` bridges the buffer's key (tuple of `telemetry_event_id`, `event_type`) to the dispatcher. `_event_id_to_buffered` dict tracks pending telemetry IDs so the aggregator's contributing-event references survive the queue hop. Dispatcher `start()` / `stop(timeout=5.0)` lifecycle: `stop()` flips `_stop_event` then joins the thread. Failures in a single dispatch are logged and skipped — the loop continues. Per-frame JPEGs and audio buffers are NOT stored on the dispatcher; the buffer holds the rolling-buffer entries and the evidence-flush route (`POST /sessions/{id}/flags/{flag_id}/evidence`) is what calls into the evidence store. | Claude |
| 2026-08-17 | **Flag persistence wiring (turn 9b).** New `src/proctoring_engine/orchestration/_flag_persistence_worker.py`: a second per-session background `threading.Thread` that drains `flag_decisions` (the dispatcher's output queue), opens a sync SQLAlchemy session via `get_db`, and calls `persist_flag_decision` to translate each `FlagDecision` into immutable `Flag` + `FlagTelemetryEvent` rows in a single transaction. Append-only invariant preserved by the existing `flag_immutable` PostgreSQL trigger. The worker accepts an `on_kill_switch(flag_id: str, reason: str)` callback that fires synchronously when `decision.triggered_termination=True` — the callback is invoked from inside the worker thread, so the consumer is responsible for any thread-to-event-loop bridging it needs (see turn 9c). `start()` / `stop(timeout=5.0)` symmetric with the dispatcher. | Claude |
| 2026-08-20 | **Kill-switch delivery + lifespan wiring (turn 9c).** Closes the loop between persistence and the client. Two pieces: (1) `_handle_kill_switch` on `FlagPersistenceWorker` upserts the 1:1 `TerminationRecord` keyed by `exam_session_id` (set `triggering_flag_id` + `reason` to the rule code, insert if missing, update if present) and then invokes the `on_kill_switch` callback. The termination write is committed before the callback fires so the kill-switch delivery never races against a missing row. (2) The WebSocket route (`src/proctoring_engine/websocket/routes.py`) constructs the `PolicySnapshot` + `SessionContext` + `FrameDispatcher` + `FlagPersistenceWorker` per accepted connection, captures the running event loop, and passes a `loop.call_soon_threadsafe(kill_switch_queue.put_nowait, (flag_id, reason))` bridge as the worker's `on_kill_switch`. A second coroutine `_start_kill_switch_drain` (returned as an `asyncio.Task`, not awaited — it self-loops on `kill_switch_queue.get()`) reads the bridge queue, calls `DeliveryService.prepare_kill_switch`, and sends the JSON envelope on the WebSocket. Both the dispatcher and worker are stopped in the route's `finally` block before the WebSocket closes. 723 Python unit tests passing. Next layer: live K8s cluster provisioning + e2e smoke test on a real cluster (turn 10). | Claude |
| 2026-08-20 | **End-to-end smoke test (turn 10).** New `tests/test_e2e_smoke.py` exercises the **full client-to-flag loop** through a real FastAPI app: a fake LTI launch (mocked OIDC discovery + JWKS via `pytest-httpx`) issues a session token, the client opens a WebSocket with that token in the `Sec-WebSocket-Protocol` header, streams three `telemetry_light` events with `face_count=2`, the aggregator fires `RULE_SECOND_PERSON` ("second_person"), the persistence worker writes the immutable `Flag` row + three `TelemetryEvent` rows + three `FlagTelemetryEvent` links + the 1:1 `TerminationRecord`, the asyncio.Queue bridge sends the `kill_switch` message over the WS, the client acks it, the `POST /sessions/{id}/flags/{flag_id}/evidence` route seals a blob through the `InMemoryEvidenceStore`, and the `EvidenceArtifact` row is committed. Eight phases, one polling helper (`_wait_for_db_row` polls with `db.expire_all()` per iteration because the persistence worker writes from a separate thread), one fresh-session-per-call `get_db` factory (the WS handler runs in a worker thread; sharing a session across threads detaches loaded instances when the worker thread closes the session). SQLite-only — does not require `INTEGRATION_DATABASE_URL`. Runs in 3.12 s. Total: 724 Python unit tests passing (723 prior + 1 new). **Two pre-existing items surfaced (not fixed in this turn):** (a) the kill-switch `payload.reason` value emitted by the Python code is `"second_person"` / `"gaze_away_frequency"` / `"accumulated_score"` (matches the `RULE_*` constants in `src/proctoring_engine/fusion/aggregator.py`) but the Python `KillSwitchPayload.reason` docstring and the TypeScript client's `REASON_MESSAGES` map (in `client/src/kill-switch.ts`) use `"second_person_detected"` / `"gaze_frequency_exceeded"` / `"accumulated_score_exceeded"` — a contract drift requiring a separate atomic turn to resolve; (b) the identity-match `_AlwaysZeroBackend` stub (`src/proctoring_engine/orchestration/_frame_dispatcher.py:835`) is an organic Turn 9a placeholder, not a designed fail-closed/block-by-default mechanism, and the designed `identity_verification_status` field that would distinguish "library not installed" from "looks like impersonation" was never built (grep-verified absent across the codebase, the docs, and the migration history) — the identity-match parked item remains parked. Next layer: live K8s cluster provisioning + e2e smoke test on a real cluster (turn 11). | Claude |
