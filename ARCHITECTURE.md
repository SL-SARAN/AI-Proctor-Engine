...
| 20 | Live cluster provisioning + e2e smoke | Pending | `docs/DEPLOYMENT.md` | — |
...
**Production deployment** (turn N+10). All client and server layers
are complete: the data model, ingestion, preprocessing, inference
(server-side and client-side), fusion, evidence, orchestration,
browser client with capture, and the client-side inference modules.
The remaining unchecked item is the live Kubernetes cluster
provisioning and end-to-end smoke test on a real cluster. The
manifests under `k8s/00-…` through `k8s/10-…` are reviewed; the
cluster itself is the next environment to provision.
