...
| 19 | Client-side inference (FaceDetector, FaceLandmarker) | Pending | `docs/04-inference-modules-design.md` | — |
...
complete (turn N+8): the WebSocket transport, browser-event capture,
in-memory rolling buffer, and kill-switch UI are all built and
tested. Turn N+9 adds `@mediapipe/tasks-vision` runtime code to the
client: the `FaceDetector` for per-frame face presence/count, and
the `FaceLandmarker` for head-pose/gaze. The `.task` model bundles
are declared in `client/package.json` and will be baked into the
Docker image. See `docs/04-inference-modules-design.md` for the
client-side inference contract.
